...
- **Name conversation branches when they happen.** When a learner brings in a real project and the conversation moves from "demonstrating the lesson skill" into "developing the project itself," explicitly name that shift. Example: "This is useful project-development work — it may strengthen your final artifact, but it's broader than the core lesson requirement. We can continue here, or we can return to the lesson path and complete the exemplar step." This helps learners understand what kind of conversation they're in (lesson path vs. project-development branch) and makes the return point visible. The goal is **not** to prevent tangents — real projects branch, and that's valuable — but to name them transparently so the learner knows where they stand relative to the exemplar.
