...
- **Accessibility is required**: every interactive element must be keyboard-operable and have an accessible name (aria-label, aria-pressed, role, etc.). Chat log uses `role="log"` `aria-live="off"`; new-message announcements go through a separate auto-clearing `role="status"` region; a streaming `AssistantMessage` is `aria-hidden` AND drops its `data-chat-message`/`tabIndex` so focus and Alt+Arrow nav can't land on an aria-hidden node; persisted messages carry sr-only speaker prefixes + `data-chat-message` for Alt+Arrow nav (`useChatKeyboardNav`). The inline `ComposeBar` (when pinned) is `inert`, not `aria-hidden`, so a focused control inside it is blurred rather than hidden-with-focus. `ComposeBar` send-key behavior is learner-configurable via a "Press Enter to send" toggle (persisted as `preferences.enterToSend`): default off → Cmd/Ctrl+Enter sends and plain Enter inserts a newline; on → Enter sends and Shift+Enter inserts a newline.
